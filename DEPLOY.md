...
Проект находится в режиме `Maintenance`. Дальнейшие изменения оформляются отдельными задачами, сначала фиксируются в Git, проходят `npm run lint` и `npm run build`, а затем развёртываются по разделу «Обычное обновление». Перед изменением схемы, данных или другой потенциально рискованной операцией создаётся свежий backup. Изменения архитектуры или порядка эксплуатации сопровождаются обновлением профильной документации.
