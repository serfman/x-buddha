...
Задача не выполняла изменений в Hostinger. Владелец подтвердил TTL 300 и A-записи `xbuddha.org` и `admin.xbuddha.org` на `178.212.14.78`, а также CNAME `www.xbuddha.org` на `xbuddha.org`; Cloudflare DNS и Google Public DNS возвращают те же значения.
