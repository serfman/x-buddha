...
| Medium | Production не отдавал базовые security headers и раскрывал application server headers. | В Next.js отключён `X-Powered-By`; Nginx скрывает upstream header, отключает version token и добавляет HSTS, nosniff, SAMEORIGIN для frontend, Referrer-Policy и Permissions-Policy. Конфигурация применяется только после `nginx -t`. |
