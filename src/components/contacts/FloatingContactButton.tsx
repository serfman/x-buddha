      ...
      {open && (
        <div className="fixed inset-0 z-[70] md:hidden" role="presentation">
          ...
          <div ref={dialogRef} role="dialog" aria-modal="true" aria-labelledby="contact-sheet-title" className="absolute inset-x-0 bottom-0 rounded-t-[2rem] border-t border-white/15 bg-[#10141c] px-5 pb-[calc(1.5rem+env(safe-area-inset-bottom))] pt-5 shadow-2xl">
            ...
            <div className="mt-5 grid gap-3">{messengers.map((messenger) => <a key={messenger.name} href={messenger.href} target="_blank" rel="noopener noreferrer" aria-label={messenger.note} onClick={() => trackMessengerClick(messenger.name, "mobile_sheet")} className="flex items-center gap-3 rounded-2xl border border-white/10 bg-white/[0.04] p-4 text-milk"><span className="grid size-9 place-items-center rounded-full border border-cold/30 text-xs font-semibold text-cold">{messenger.short}</span>{messenger.name}</a>)}</div>
          </div>
        </div>
      )}
    </>
